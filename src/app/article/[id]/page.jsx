  ...
  return (
    <main style={{ maxWidth: '900px', margin: '40px auto', padding: '0 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        ...
        <div className="category-tag" style={{ margin: 0 }}>
          <Sparkles size={13} />
      ...
      {/* Article Headline */}
      <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: '38px', lineHeight: 1.2, fontWeight: 900, color: 'var(--text-primary)', marginBottom: '16px' }}>
        {article.title}
      </h1>

      {/* Subtitle / Excerpt */}
      {article.subtitle && (
        <p style={{ fontSize: '18px', color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: '24px', fontStyle: 'italic' }}>
          {article.subtitle}
        </p>
      )}

      {/* Author & Action Bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid var(--border-color)', borderBottom: '1px solid var(--border-color)', padding: '16px 0', marginBottom: '32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          <div>
            <div style={{ fontWeight: 800, color: 'var(--text-primary)', fontSize: '15px' }}>{article.author}</div>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span>{article.date || 'August 3, 2026'}</span>
              <span>•</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <button style={{ padding: '8px', borderRadius: 'var(--radius-sm)', background: 'var(--bg-secondary)', color: 'var(--text-secondary)' }}>
            <Share2 size={18} />
          </button>
        </div>
      </div>
